...
! On most system need to increase stack size first, ulimit -s unlimited
  ...
  ! cos phi *cos psi in Dmumu, ! delta-like injection !solar wind speed 413km/s !Add solar wind convection?
  ...
  REAL(KIND=8), PARAMETER :: PI = 3.14159, time_max = 15.
  ...
  REAL(KIND=8) :: acceleration_time = 0.8, escape_time = 2.2 
  REAL(KIND=8) :: injection_broadness = 24./180.*PI, phi_source = 90./180*PI
 ...
 Earth_phi = 27./180.*PI - 4./180.*PI !0/180.*PI
 Earth_r = 1.0
 
 Mars_phi = 169.0638746/180.*PI
 Mars_r = 1.610973075
 
 STA_phi = 27./180.*PI + 86./180.*PI ! 65./180.*PI
 STA_r = 1.0

 STB_phi = 27./180.*PI - 50./180.*PI !-71./180.*PI
 STB_r = 1.0
...
END DO
   ...
   f_new(1,j,k) = f_old(1,j,k)  
    ...
    if (time.LT.20.*minute) then
   
   f_old(1,j,k) = 10.*exp(-(PHI(j) - phi_source)*(PHI(j) - phi_source)/2./injection_broadness/injection_broadness) + 0.0001
   f_new(1,j,k) = f_old(1,j,k)  
...
END DO
 ...
 DO i = 1, N
   ...
   DO k = 1, NK
    ...
    Kappa_perp(i,k) = speed/3.*0.04*0.1*R(i)*R(i)*cospsi(i)/cospsi(i)/cospsi(i)*SQRT(1. - mu(k)*mu(k))
    !speed/3.*lambda_perp(i)*ABS(MU(k))*2.
