...
! On most system need to increase stack size first, ulimit -s unlimited
  ...
  ! Circumsolar injection, perp diffusion 0., cos phi *cos psi in Dmumu
  
  use omp_lib
   
  IMPLICIT NONE
  
  INTEGER, PARAMETER :: N = 150, M = 100, NK = 99
  INTEGER :: i, j, k, indeks_r, indeks_phi, onset_time_flag(M), species
  
  REAL(KIND=8), PARAMETER :: PI = 3.14159, time_max = 10.
  ...
  REAL(KIND=8) :: injection_broadness = 5./180.*PI, acceleration_time = 0.8, escape_time = 2.2 
  REAL(KIND=8) :: phi_source = 2./180*PI
...
END DO
   ...
   f_new(1,j,k) = f_old(1,j,k)  
    ...
    if (time.LT.minute) then
   
   f_old(1,j,k) = 1000000. + 0.0001
   f_new(1,j,k) = f_old(1,j,k)  
...
IF (time_printer.GE.minute) THEN
 ...
 !Earth
 WRITE(*,*) 'Earth'
  indeks_phi = (2.35E-06/180.*PI - phi(1) + delta_phi)/delta_phi + 1.
  indeks_r = (0.993490032 - R(1) + delta_r)/delta_r + 1.
...
!-------------------------------------------------------------------------------------
 !Mars
  WRITE(*,*) 'Mars'
  indeks_phi = (169.0638746/180.*PI - phi(1) + delta_phi)/delta_phi + 1.
  indeks_r = (1.610973075 - R(1) + delta_r)/delta_r + 1.
...
!STA
 WRITE(*,*) 'STA '
  indeks_phi = (-37.59619789/180.*PI - phi(1) + delta_phi)/delta_phi + 1.
  indeks_r = (0.958293441 - R(1) + delta_r)/delta_r + 1.
...
!Bepi
 WRITE(*,*) 'Bepi '
  indeks_phi = (89.94839563/180.*PI - phi(1) + delta_phi)/delta_phi + 1.
  indeks_r = (0.411289483 - R(1) + delta_r)/delta_r + 1.
...
!PSP
 WRITE(*,*) 'PSP '
  indeks_phi = (-53.64955116/180.*PI - phi(1) + delta_phi)/delta_phi + 1.
  indeks_r = (0.623189194 - R(1) + delta_r)/delta_r + 1.
...
!SolO
 WRITE(*,*) 'SolO'
  indeks_phi = (-2.819722862/180.*PI - phi(1) + delta_phi)/delta_phi + 1.
  indeks_r = (0.803862906 - R(1) + delta_r)/delta_r + 1.
...
!Boundary
  WRITE(*,*) 'Boundary'
  indeks_phi = (phi_source/180.*PI - phi(1) + delta_phi)/delta_phi + 1.
  indeks_r = 1.
...
time_end - time_begin, ' seconds'     
 ...
 V_sw = 400./1.5d8*(60*60) !AU/hr
  ...
  DO i = 1, N
    ...
    tanpsi = (R(i) - 0.0)*Omega/V_sw ! no units, of course
...
DO i = 1, N
  ...
  DO k = 1, NK
    ...
    F(i,k) = (1. - mu(k)*mu(k))*((ABS(MU(k)))**(1.67 - 1.) + 0.05)
 ...
 DO i = 1, N
  ...
  DO k = 1, NK
    ...
    F(i,k) = F(i,k)*integral*3.*speed/8./lambda(i)*cospsi(i)*cospsi(i)
...
END DO
 ...
 integral = 0.
  ...
  lambda_perp(i) = 0.5*integral*ABS(mu(1) - mu(2))
