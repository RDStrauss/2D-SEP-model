...
! -----------------------------------------------------------------------------------   
 ...
 ! Z_max = maximum extent of the Z-axis
  ...
  INTEGER, PARAMETER :: N = 150, M = 99
  INTEGER :: i,j, zlimiter, mulimiter, z_index, species, writer(5), injection_swtich
  REAL(KIND=8) :: Delta_t, Delta_z, Delta_mu, CFL_coeff, V_sw, D_mumu_max, D_mumu(N,M)
  REAL(KIND=8) :: Z(N), MU(M), time, totaltime, L_max, anisotropy, D_mumu_dmu(N,M), Z_max = 3
  REAL(KIND=8) :: f(N,M), f0(N,M), f00(N,M), L(N), f_omni_direc, time_printer, acceleration_time
  REAL(KIND=8) :: A(M), B(N,M), speed, left_lim, right_lim, limiter, B_max, r_printer, escape_time
  REAL(KIND=8) :: energy, lambda, times(5), pitch_angle_distribution(5,M), r_position
...
!3 = superbee
 ...
 IF (injection_swtich.EQ.2) THEN
   ...
   DO i = 1, N
    ...
    ! This small Gaussian approximates a delta-like injecion in space
    f0(i,j) = f0(i,j) + 100.*exp(-(Z(i) - 0.08)*(Z(i) - 0.08)/0.05)/time*exp(-acceleration_time/time - time/escape_time)*Delta_t
...
!-----------------------------------------------------------------------
 ...
 time_printer = time_printer - 0.01667
 f_omni_direc = 0.
    
    DO j = 1, M

! integrate the distribution function to find the omni-directional intensity
    f_omni_direc = f_omni_direc + f(z_index,j) + 0.000001
...
! integrate the distribution function * mu to find the omni-directional intensity    
    anisotropy = anisotropy + 3.*(f(z_index,j) + 0.000001)*MU(j)
...
SUBROUTINE DEF_COEFFICIENTS(speed,N,L,Z,M,MU,B,D_mumu,D_mumu_dmu,A,z_index,energy,lambda,species, V_sw,r_position,r_printer)
 ...
 REAL(KIND=8) :: speed, L(N), Z(N), D_0, MU(M), B(N,M), D_mumu(N,M), D_mumu_dmu(N,M), A(M)
 REAL(KIND=8) :: V_sw, Omega, R(N), SunBeta, r0, delta_r, test, integral, psi, beta
 REAL(KIND=8) :: energy, lambda, rigidity, r_position, r_printer, rest_energy
 REAL(KIND=8), PARAMETER :: speed_of_light = 7.2093d0 !in units of AU/hr
 REAL(KIND=8) :: min_vector(N) ! vector used to minimize difference to find index of r_position
 REAL(KIND=8), PARAMETER :: PI = 3.14159
 
 V_sw = V_sw/1.5d8*60.*60 !solar wind speed in AU/hr
 Omega = 2.*PI/25.4/24. !solar rotation rate in /AU
 ...
 ! DO the derivative of D_mumu numerically
  DO i = 1, N
   ...
   END DO
     ...
     D_mumu_dmu(i,1) = (D_mumu(i,2) - D_mumu(i,1))/ABS(MU(2) - MU(1))
