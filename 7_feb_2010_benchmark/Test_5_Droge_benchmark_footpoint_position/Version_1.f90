...
! On most system need to increase stack size first, ulimit -s unlimited
  ...
  REAL(KIND=8) :: injection_broadness = 26./180.*PI, phi_source = 90./180*PI
...
END DO
 ...
 DO i = 1, N
   ...
   DO k = 1, NK
    ...
    Kappa_perp(i,k) = speed/3.*0.13*0.08*R(i)*R(i)*cospsi(i)/cospsi(i)/cospsi(i)*SQRT(1. - mu(k)*mu(k))
