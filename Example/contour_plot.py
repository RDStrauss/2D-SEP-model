...
Mars_r = np.array([data[0,0],data[0,0]])
Mars_phi = np.array([data[0,1],data[0,1]])

data = np.loadtxt("Bepi_omni_time.txt")
Bepi_r = np.array([data[0,0],data[0,0]])
Bepi_phi = np.array([data[0,1],data[0,1]])

data = np.loadtxt("PSP_omni_time.txt")
PSP_r = np.array([data[0,0],data[0,0]])
PSP_phi = np.array([data[0,1],data[0,1]])

data = np.loadtxt("SolO_omni_time.txt")
SolO_r = np.array([data[0,0],data[0,0]])
SolO_phi = np.array([data[0,1],data[0,1]])
...
V_sw = 400./1.5e8*60.*60 #units of vsw in AU/hr
...
plt.scatter(Mars_x, Mars_y, facecolor = 'black', marker = 's', s = 50, edgecolor = 'white')
phi_HMF = -(Mars_r[0] - 0.05)/V_sw*Omega
dphi = Mars_phi[0] - phi_HMF
plt.plot(r*np.cos(phi + dphi), r*np.sin(phi + dphi), color = 'black', linestyle = '-', linewidth = 2, label = 'Mars')
...
phi_HMF = -(Bepi_r[0] - 0.05)/V_sw*Omega
dphi = Bepi_phi[0] - phi_HMF
plt.plot(r*np.cos(phi + dphi), r*np.sin(phi + dphi), color = 'orange', linestyle = '-', linewidth = 2, label = 'Bepi')
...
phi_HMF = -(PSP_r[0] - 0.05)/V_sw*Omega
dphi = PSP_phi[0] - phi_HMF
plt.plot(r*np.cos(phi + dphi), r*np.sin(phi + dphi), color = 'purple', linestyle = '-', linewidth = 2, label = 'PSP')

# SolO observer
SolO_x = SolO_r*np.cos(SolO_phi)
SolO_y = SolO_r*np.sin(SolO_phi)
plt.scatter(SolO_x, SolO_y, facecolor = 'gray', marker = 's', s = 50, edgecolor = 'white')
phi_HMF = -(SolO_r[0] - 0.05)/V_sw*Omega
dphi = SolO_phi[0] - phi_HMF
plt.plot(r*np.cos(phi + dphi), r*np.sin(phi + dphi), color = 'gray', linestyle = '-', linewidth = 2, label = 'SolO')
